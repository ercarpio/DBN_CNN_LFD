...
class DQNModel:
	...
	def __init__(self, graphbuild = [1]*TOTAL_PARAMS, batch_size=1, filename="", name="dqn", learning_rate=1e-4, log_dir=""):
		...
		self.variables_aud = {
			"W1" : weight_variable("W_conv1_aud", [aud_filter_sizes[0][0],aud_filter_sizes[0][1],aud_dtype["num_c"],aud_layer_elements[1]]),
			"b1" : bias_variable("b_conv1_aud", [aud_layer_elements[1]]),
			"W2" : weight_variable("W_conv2_aud", [aud_filter_sizes[1][0],aud_filter_sizes[1][1],aud_layer_elements[1],aud_layer_elements[2]]),
			"b2" : bias_variable("b_conv2_aud", [aud_layer_elements[2]]),
			"W3" : weight_variable("W_conv3_aud", [aud_filter_sizes[2][0],aud_filter_sizes[2][1],aud_layer_elements[2],aud_layer_elements[3]]),
		...
		self.variables_pnt_hat = {
			"W1" : weight_variable("W_conv1_pnt_hat", [filter_sizes[0],filter_sizes[0],pnt_dtype["num_c"],layer_elements[1]]),
			"b1" : bias_variable("b_conv1_pnt_hat", [layer_elements[1]]),
			"W2" : weight_variable("W_conv2_pnt_hat", [filter_sizes[1],filter_sizes[1],layer_elements[1],layer_elements[2]]),
			"b2" : bias_variable("b_conv2_pnt_hat", [layer_elements[2]]),
			"W3" : weight_variable("W_conv3_pnt_hat", [filter_sizes[2],filter_sizes[2],layer_elements[2],layer_elements[-2]]),
			"b3" : bias_variable("b_conv3_pnt_hat", [layer_elements[-2]])#,
		}
		
		self.variables_aud_hat = {
			"W1" : weight_variable("W_conv1_aud_hat", [aud_filter_sizes[0][0],aud_filter_sizes[0][1],aud_dtype["num_c"],aud_layer_elements[1]]),
			"b1" : bias_variable("b_conv1_aud_hat", [aud_layer_elements[1]]),
			"W2" : weight_variable("W_conv2_aud_hat", [aud_filter_sizes[1][0],aud_filter_sizes[1][1],aud_layer_elements[1],aud_layer_elements[2]]),
			"b2" : bias_variable("b_conv2_aud_hat", [aud_layer_elements[2]]),
			"W3" : weight_variable("W_conv3_aud_hat", [aud_filter_sizes[2][0],aud_filter_sizes[2][1],aud_layer_elements[2],aud_layer_elements[3]]),
		...
		self.optimizer = tf.train.AdamOptimizer(learning_rate=self.__alpha).minimize(self.cross_entropy, var_list=variables_to_train)
...
#####################
	...
	def model(self, seq_length, img_ph, pnt_ph, aud_ph, partitions_ph, train_ph, prompts_ph, variable_scope, variable_scope2, var_img, var_pnt, var_aud, var_lstm, incep_reuse=True):#
		...
		def convolve_data_3layer_pnt(input_data, val, variables, n, dtype):
			...
			input_data = gen_convolved_output(input_data, variables["W1"], variables["b1"], stride_sizes[0], layer_elements[1], output_sizes[0], train_ph, padding)
			...
			input_data = gen_convolved_output(input_data, variables["W2"], variables["b2"], stride_sizes[1], layer_elements[2], output_sizes[1], train_ph, padding)
			...
			input_data = gen_convolved_output(input_data, variables["W3"], variables["b3"], stride_sizes[-1], layer_elements[-2], output_sizes[-1], train_ph, padding)
		...
		def convolve_data_3layer_aud(input_data, val, variables, n, dtype):
			...
			input_data = gen_convolved_output(input_data, variables["W1"], variables["b1"], aud_stride_sizes[0], aud_layer_elements[1], aud_output_sizes[0], train_ph, padding)
			self.variable_summaries(input_data, dtype["name"]+"_conv1")
			input_data = tf.verify_tensor_all_finite(
				input_data,
			...
			input_data = gen_convolved_output(input_data, variables["W2"], variables["b2"], aud_stride_sizes[1], aud_layer_elements[2], aud_output_sizes[1], train_ph, padding)
			self.variable_summaries(input_data, dtype["name"]+"_conv2")
			input_data = tf.verify_tensor_all_finite(
				input_data,
			...
			input_data = gen_convolved_output(input_data, variables["W3"], variables["b3"], aud_stride_sizes[2], aud_layer_elements[3], aud_output_sizes[2], train_ph, padding)
			self.variable_summaries(input_data, dtype["name"]+"_conv3")
			input_data = tf.verify_tensor_all_finite(
				input_data,
